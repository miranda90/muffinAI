		...
		if( !empty($wrap['attr']) && is_iterable($wrap['attr']) ) {
			...
			foreach ($wraps_fields as $wf) {
				...
				if( strpos($wf['id'], 'css_') === false ) {
					...
					if( !empty($wf['old_id']) && isset($this->mfn_items[$this->s]['wraps'][$this->w]['attr'][$wf['old_id']] ) ) {
						// if style: for dekstop
						if( !empty($wf['responsive']) ) {
							$item_obj['val']['desktop'] = $this->mfn_items[$this->s]['wraps'][$this->w]['attr'][$wf['old_id']];
						}else{
							$item_obj['val'] = $this->mfn_items[$this->s]['wraps'][$this->w]['attr'][$wf['old_id']];
						}
		...
		if( !empty($wrap['attr']) && is_iterable($wrap['attr']) ) {
			...
			foreach ($wraps_fields as $wf) {
				...
				if( strpos($wf['id'], 'css_') === false ){
					...
					if( !empty($wf['old_id']) && isset($this->mfn_items[$this->s]['wraps'][$this->w]['items'][$this->nw]['attr'][$wf['old_id']] ) ) {
						// if style: for dekstop
						if( !empty($wf['responsive']) ) {
							$item_obj['val']['desktop'] = $this->mfn_items[$this->s]['wraps'][$this->w]['items'][$this->nw]['attr'][$wf['old_id']];
						}else{
							$item_obj['val'] = $this->mfn_items[$this->s]['wraps'][$this->w]['items'][$this->nw]['attr'][$wf['old_id']];
						}
